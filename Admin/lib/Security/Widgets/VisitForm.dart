...
class VisitForm extends StatefulWidget {
  ...
  const VisitForm({Key key, @required String docId})
      : assert(docId != null),
        _docId = docId,
        super(key: key);
  @override
  State<StatefulWidget> createState() => _VisitFormState();
}

class _VisitFormState extends State<VisitForm> {
  SecurityCheckBloc _securityCheckBloc;
  Image pickedImage;
  @override
  void initState() {
    _securityCheckBloc = BlocProvider.of<SecurityCheckBloc>(context);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<SecurityCheckBloc, SecurityCheckState>(
        builder: (context, state) {
      if (state is SecurityCheckLoading) {
        return LoadingIndicator();
      }
      // else if (state is SecurityCheckLoaded) {
      //   final secCheck = state.securityCheck.values;
      //   return Center(
      //     child: Text("reach" + secCheck.toList().toString()),
      //   );
      // }
      else if (state is DocIDCheckLoaded) {
        print("reach in doc loaded state");
  ...
  void _onFormSubmitted_Exit(int entryTime) async {
    try {
      _securityCheckBloc.add(
        UpdateSecurityCheckVisit(SecurityCheck(
            doc_id: widget._docId,
            status: "completed",
            entryTime: entryTime,
