...
class VisitBloc extends Bloc<VisitEvent, VisitState> {
  ...
  Stream<VisitState> _mapToggleAllToState() async* {
    ...
    if (currentState is VisitLoaded) {
      ...
      final allComplete =
          currentState.visit.every((visit) => visit.status == "req");
      final List<Visit> updatedVisit = currentState.visit
          .map((visit) => visit.copyWith(status: allComplete.toString()))
          .toList();
  ...
  Stream<VisitState> _mapClearCompletedToState() async* {
    final currentState = state;
    if (currentState is VisitLoaded) {
      final List<Visit> completedVisit = currentState.visit
